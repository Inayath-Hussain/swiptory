...
const controller: RequestHandler<{}, {}, ILikeStoryBody> = async (req, res, next) => {
    ...
    try {
        const storyDoc = await storyService.unlikeStory(story_id, session);

        if (storyDoc === null) throw new ServiceError("story doesn't exist");

        const userLikedStoriesDoc = await userLikedStoriesService.removeStory(user_id, storyDoc._id, session);
...
export const unlikeStoryController = tryCatchWrapper(controller);